...
namespace tt {
	...
	struct visualizer final {
		...
		inline tt_string visualize(const Value&) const {
...
// Specializes tt::visualizer for CLASS such that it will invoke CLASS's visualize function when used.
...
\
	inline tt_string visualize(const CLASS& x) const { \
\
\
		return x.visualize(); \
	} \
...
// A version of TT_REGISTER_VISUALIZE which defines the body of the visualizer's visualize function explicitly.
...
\
	using this_t = tt::visualizer<CLASS>; \
\
	inline tt_string visualize(const CLASS& VALUE) const BODY \
}
...
\
	inline tt_string visualize(const CLASS ARGS& x) const { \
\
\
		return x.visualize(); \
	} \
...
// A version of TT_REGISTER_VISUALIZE_T which defines the body of the visualizer's visualize function explicitly.
...
\
	using this_t = tt::visualizer<CLASS ARGS>; \
\
	inline tt_string visualize(const CLASS ARGS& VALUE) const BODY \
}
